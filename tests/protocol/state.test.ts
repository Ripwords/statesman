...
describe('state service', () => {
  ...
  it('round-trips a written state', async () => {
    await writeState({ projectId, orgSlug: ORG, projectSlug: 'prod', body: payload(1), userId: 'u1' })
    expect(await readCurrentState(projectId)).toEqual(payload(1))
  })

  it('keeps every version and moves the pointer', async () => {
    await writeState({ projectId, orgSlug: ORG, projectSlug: 'prod', body: payload(1), userId: 'u1' })
    await writeState({ projectId, orgSlug: ORG, projectSlug: 'prod', body: payload(2), userId: 'u1' })
  ...
  it('records serial and lineage for display', async () => {
    await writeState({ projectId, orgSlug: ORG, projectSlug: 'prod', body: payload(7), userId: 'u1' })
  ...
  it('accepts state that is not valid JSON', async () => {
    ...
    await writeState({ projectId, orgSlug: ORG, projectSlug: 'prod', body: junk, userId: 'u1' })
